...
export default function RechartsGroupedBar({ targetMonth }: Props) {
  ...
  const filteredData = useMemo(
    () => targetMonth ? inputData.filter(d => d.date.startsWith(targetMonth)) : inputData,
    [targetMonth]
  );
  ...
  const grouped: { [name: string]: { region: string; sales: number } } = {};
  ...
  const result = Object.entries(grouped).map(([name, { region, sales }]) => ({
    name,
    region,
    sales
  }));
  ...
  if (mode === 'grouped') {
    ...
    return regions.flatMap(region =>
      result.filter(d => d.region === region).sort((a, b) => b.sales - a.sales)
    );
  } else {
  ...
  return (
    <div style={{ width: '100%', height: 500 }}>
      <Button variant="outlined" style={{ marginBottom: "1rem" }} onClick={() => setMode(mode === 'grouped' ? 'flat' : 'grouped')}>
        モード切替（現在: {mode === 'grouped' ? '部門別ソート' : '全店舗ソート'}）
      </Button>
      <h2 style={{color: "#B71C1C"}}>{targetMonth ? `${targetMonth} の集計データ` : "全期間のデータ"}</h2>
      ...
      <ResponsiveContainer>
        ...
        >
          ...
          <XAxis dataKey={targetMonth ? "name" : "name"} interval={0} />
          <YAxis />
          <Tooltip />
          <Legend />
          ...
          {mode === 'grouped' ? (
            regions.map((region, i) => (
              <Bar
                key={region}
                dataKey={(entry: any) => entry.region === region ? entry.sales : 0}
                name={region}
